...
`define INST_MULHU_MASK 32'hfe00707f
    ...
    assign final_result_s = (upper2_reg==1'b1) ? mult_result_s[6][63:32] : mult_result_s[6][31:0];

    always@(posedge clk_i, posedge rst_i) begin
        if (rst_i) begin 
            pipe_stage3_s <= 32'h00000000;
        end
        else if (clk_i && hold_i) begin
            pipe_stage3_s <= final_result_s;
        end
    end

    assign stall_o = stall_s_p2_r;
    assign writeback_value_o = pipe_stage3_s;
