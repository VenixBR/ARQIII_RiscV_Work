...
(
    ...
    generate
        for (i = 0; i < 64; i = i + 1) begin
            assign ext_mult_s[i] = {{56{AxB_8b_s[i][7]}}, AxB_8b_s[i]} ;
        end
    endgenerate
    ...
    // assign sft_mult_s[0] = AxB_8b_s[0];     // A0 + B0
    // assign sft_mult_s[1] = AxB_8b_s[1]<<4;  // A0 + B1<<4
    // assign sft_mult_s[2] = AxB_8b_s[2]<<8;  // A0 + B2<<8
    // assign sft_mult_s[3] = AxB_8b_s[3]<<12; // A0 + B3<<12
    // assign sft_mult_s[4] = AxB_8b_s[4]<<16; // A0 + B4<<16
    // assign sft_mult_s[5] = AxB_8b_s[5]<<20; // A0 + B5<<20
    // assign sft_mult_s[6] = AxB_8b_s[6]<<24; // A0 + B6<<24
    // assign sft_mult_s[7] = AxB_8b_s[7]<<28; // A0 + B7<<28

    // assign sft_mult_s[8]  = AxB_8b_s[8]<<4;    // A0<<4 + B0
    // assign sft_mult_s[9]  = AxB_8b_s[9]<<8;    // A0<<4 + B1<<4
    // assign sft_mult_s[10] = AxB_8b_s[10]<<12;  // A0<<4 + B2<<8
    // assign sft_mult_s[11] = AxB_8b_s[11]<<16;  // A0<<4 + B3<<12
    // assign sft_mult_s[12] = AxB_8b_s[12]<<20;  // A0<<4 + B4<<16
    // assign sft_mult_s[13] = AxB_8b_s[13]<<24;  // A0<<4 + B5<<20
    // assign sft_mult_s[14] = AxB_8b_s[14]<<28;  // A0<<4 + B6<<24
    // assign sft_mult_s[15] = AxB_8b_s[15]<<32;  // A0<<4 + B7<<28

    // assign sft_mult_s[16] = AxB_8b_s[16]<<8;    // A0<<8 + B0
    // assign sft_mult_s[17] = AxB_8b_s[17]<<12;   // A0<<8 + B1<<4
    // assign sft_mult_s[18] = AxB_8b_s[18]<<16;   // A0<<8 + B2<<8
    // assign sft_mult_s[19] = AxB_8b_s[19]<<20;   // A0<<8 + B3<<12
    // assign sft_mult_s[20] = AxB_8b_s[20]<<24;   // A0<<8 + B4<<16
    // assign sft_mult_s[21] = AxB_8b_s[21]<<28;   // A0<<8 + B5<<20
    // assign sft_mult_s[22] = AxB_8b_s[22]<<32;   // A0<<8 + B6<<24
    // assign sft_mult_s[23] = AxB_8b_s[23]<<36;   // A0<<8 + B7<<28

    // assign sft_mult_s[24] = AxB_8b_s[24]<<12;    // A0<<12 + B0
    // assign sft_mult_s[25] = AxB_8b_s[25]<<16;   // A0<<12 + B1<<4
    // assign sft_mult_s[26] = AxB_8b_s[26]<<20;   // A0<<12 + B2<<8
    // assign sft_mult_s[27] = AxB_8b_s[27]<<24;   // A0<<12 + B3<<12
    // assign sft_mult_s[28] = AxB_8b_s[28]<<28;   // A0<<12 + B4<<16
    // assign sft_mult_s[29] = AxB_8b_s[29]<<32;   // A0<<12 + B5<<20
    // assign sft_mult_s[30] = AxB_8b_s[30]<<36;   // A0<<12 + B6<<24
    // assign sft_mult_s[31] = AxB_8b_s[31]<<40;   // A0<<12 + B7<<28

    // assign sft_mult_s[32] = AxB_8b_s[32]<<16;   // A0<<16 + B0
    // assign sft_mult_s[33] = AxB_8b_s[33]<<20;   // A0<<16 + B1<<4
    // assign sft_mult_s[34] = AxB_8b_s[34]<<24;   // A0<<16 + B2<<8
    // assign sft_mult_s[35] = AxB_8b_s[35]<<28;   // A0<<16 + B3<<12
    // assign sft_mult_s[36] = AxB_8b_s[36]<<32;   // A0<<16 + B4<<16
    // assign sft_mult_s[37] = AxB_8b_s[37]<<36;   // A0<<16 + B5<<20
    // assign sft_mult_s[38] = AxB_8b_s[38]<<40;   // A0<<16 + B6<<24
    // assign sft_mult_s[39] = AxB_8b_s[39]<<44;   // A0<<16 + B7<<28

    // assign sft_mult_s[40] = AxB_8b_s[40]<<20;   // A0<<20 + B0
    // assign sft_mult_s[41] = AxB_8b_s[41]<<24;   // A0<<20 + B1<<4
    // assign sft_mult_s[42] = AxB_8b_s[42]<<28;   // A0<<20 + B2<<8
    // assign sft_mult_s[43] = AxB_8b_s[43]<<32;   // A0<<20 + B3<<12
    // assign sft_mult_s[44] = AxB_8b_s[44]<<36;   // A0<<20 + B4<<16
    // assign sft_mult_s[45] = AxB_8b_s[45]<<40;   // A0<<20 + B5<<20
    // assign sft_mult_s[46] = AxB_8b_s[46]<<44;   // A0<<20 + B6<<24
    // assign sft_mult_s[47] = AxB_8b_s[47]<<48;   // A0<<20 + B7<<28

    // assign sft_mult_s[48] = AxB_8b_s[48]<<24;   // A0<<24 + B0
    // assign sft_mult_s[49] = AxB_8b_s[49]<<28;   // A0<<24 + B1<<4
    // assign sft_mult_s[50] = AxB_8b_s[50]<<32;   // A0<<24 + B2<<8
    // assign sft_mult_s[51] = AxB_8b_s[51]<<36;   // A0<<24 + B3<<12
    // assign sft_mult_s[52] = AxB_8b_s[52]<<40;   // A0<<24 + B4<<16
    // assign sft_mult_s[53] = AxB_8b_s[53]<<44;   // A0<<24 + B5<<20
    // assign sft_mult_s[54] = AxB_8b_s[54]<<48;   // A0<<24 + B6<<24
    // assign sft_mult_s[55] = AxB_8b_s[55]<<52;   // A0<<24 + B7<<28

    // assign sft_mult_s[56] = AxB_8b_s[56]<<28;   // A0<<28 + B0
    // assign sft_mult_s[57] = AxB_8b_s[57]<<32;   // A0<<28 + B1<<4
    // assign sft_mult_s[58] = AxB_8b_s[58]<<36;   // A0<<28 + B2<<8
    // assign sft_mult_s[59] = AxB_8b_s[59]<<40;   // A0<<28 + B3<<12
    // assign sft_mult_s[60] = AxB_8b_s[60]<<44;   // A0<<28 + B4<<16
    // assign sft_mult_s[61] = AxB_8b_s[61]<<48;   // A0<<28 + B5<<20
    // assign sft_mult_s[62] = AxB_8b_s[62]<<52;   // A0<<28 + B6<<24
    // assign sft_mult_s[63] = AxB_8b_s[63]<<56;   // A0<<28 + B7<<28
    ...
    assign mult_result_s = sft_mult_s[0] + sft_mult_s[1] + sft_mult_s[2] + sft_mult_s[3] +
                           ...
                           sft_mult_s[36] + sft_mult_s[37] + sft_mult_s[38] + sft_mult_s[39] +
                           sft_mult_s[40] + sft_mult_s[41] + sft_mult_s[42] + sft_mult_s[43] +
                           sft_mult_s[44] + sft_mult_s[45] + sft_mult_s[46] + sft_mult_s[47] +
                           sft_mult_s[48] + sft_mult_s[49] + sft_mult_s[50] + sft_mult_s[51] +
                           sft_mult_s[52] + sft_mult_s[53] + sft_mult_s[54] + sft_mult_s[55] +
                           sft_mult_s[56] + sft_mult_s[57] + sft_mult_s[58] + sft_mult_s[59] +
                           sft_mult_s[60] + sft_mult_s[61] + sft_mult_s[62] + sft_mult_s[63];
